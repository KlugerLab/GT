generate_tree <- function(maxT = 10,
...
gc_mat_tree <- generate_tree(maxT = 15)

dim(gc_mat_tree)
max(gc_mat_tree)

set.seed(1)
pheatmap::pheatmap(log(gc_mat_tree+1), cluster_cols = F, cluster_rows = F, color = viridis(100), show_rownames = F, show_colnames = F)

#pheatmap::pheatmap(log(gc_mat_tree+1), cluster_rows = F, cluster_cols = F, show_rownames = F, show_colnames = F)

data_S <- CreateSeuratObject(gc_mat_tree)
...
DimPlot(data_S, reduction = "pca", dims = c(1,3))
...
FeaturePlot(data_S,  features = rownames(data_S)[c(51, 251, 450, 625, 700)], ncol = 5, pt.size = 2.5, reduction = "tsne", order = F) & scale_color_viridis(option = "D") & NoAxes() #& NoLegend()
...
data_S <- RunUMAP(
  data_S, dims = 1:30, a = 10, b = 10#n.neighbors = 500, min.dist = 2#,  , repulsion.strength = 10
)
DimPlot(data_S, reduction = "umap", cols = "black", pt.size = 1.5) & NoLegend() & NoAxes()

FeaturePlot(data_S, features = rownames(data_S)[c(51, 251, 450, 625, 700)], ncol = 5, pt.size = 1, reduction = "umap", order = F) & scale_color_viridis(option = "D") & NoAxes() #& NoLegend()
FeaturePlot(data_S, features = rownames(data_S)[c(51, 251, 450, 625, 700)], ncol = 5, pt.size = 1.5, reduction = "dm", order = F) & 
  scale_color_viridis(option = "D") & NoAxes() &
  xlim(min(data_S@reductions$dm@cell.embeddings[,1]), max(data_S@reductions$dm@cell.embeddings[,1])) & #& NoLegend()
  ylim(min(data_S@reductions$dm@cell.embeddings[,2]), max(data_S@reductions$dm@cell.embeddings[,2]))
...
data_S_tree_v4 <- data_S
data_S_tree_v5 -> data_S
...
################################################
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_tree_dm5_k15_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_tree_pca10_k10_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_tree_dm30_k10_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_tree_dm5_k10_N", 500, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_tree_dm5_k25_N", 500, "/")
if (T){
...
umap.output <- umap(emd_mat, n_neighbors = 200, min_dist = 0.5, input="dist")
umap.output <- umap(emd_mat, a = 5, b = 2.5, input="dist")
...
genes <- rownames(data_S)[c(51, 251, 450, 625, 700)]
...
df_plot <- as.data.frame(cbind(tsne_emb$Y, as.numeric(rownames(emd_mat))))

#df_plot <- as.data.frame(cbind(gene_embedding[,c(1,2)], as.numeric(rownames(emd_mat))))
colnames(df_plot) <- c("x", "y", "gene_PT")
ggplot(data = df_plot, aes(x=x, y=y, color=gene_PT)) +
  geom_point() & scale_color_viridis(option = "C") &
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
