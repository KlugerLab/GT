generate_linear <- function(maxT = 10,
                            N_cells = 1000,
                            N_genes = 200,
...
gc_mat_linear <- generate_linear(maxT = 15)

dim(gc_mat_linear)
max(gc_mat_linear)

set.seed(1)
pheatmap::pheatmap(log(gc_mat_linear+1), cluster_cols = F, cluster_rows = F, color = viridis(100), show_rownames = F, show_colnames = F)

#pheatmap::pheatmap(log(gc_mat_linear+1), cluster_rows = F, cluster_cols = F, show_rownames = F, show_colnames = F)

data_S <- CreateSeuratObject(gc_mat_linear)
...
data_S <- RunTSNE(
  ...
  fast_tsne_path="/home/jz437/git/FIt-SNE/bin/fast_tsne", ann_not_vplinear=FALSE, nthreads=12
)
...
data_S <- RunUMAP(
  data_S, dims = 1:30, n.neighbors = 250, min.dist = 1#, a = 5, b = 10, repulsion.strength = 5
)
DimPlot(data_S, reduction = "umap", cols = "black") & NoLegend() & NoAxes()

FeaturePlot(data_S, features = rownames(data_S)[c(1,51,101,151,200)], ncol = 5, pt.size = 1, reduction = "umap", order = F) & scale_color_viridis(option = "D") & NoAxes() #& NoLegend()

FeaturePlot(data_S, features = "cell_pseudotime_linear", reduction = "umap", order = F) & scale_color_viridis(option = "B") & NoAxes()
FeaturePlot(data_S, features = "cell_pseudotime_CC", reduction = "umap", order = F) & scale_color_viridis(option = "B") & NoAxes()

data_S_linear_v3 -> data_S
#data_S_linear_v3 -> data_S
...
################################################
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_linear_dm5_k15_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_linear_pca10_k10_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_linear_dm30_k10_N", 1000, "/")
dir.path <- paste0("/data/rihao/project_with_xiuyuan/simulation/", "new_linear_dm5_k10_N", 500, "/")
if (T){
...
genes <- c(rownames(data_S)[c(1,51,101,151,200)])
